...
class TestTranscriptionEngine:
    ...
    def test_progress_callback_during_initialization(self, mock_torch, mock_processor, mock_model):
        ...
        callback.assert_any_call({"status": "initializing", "message": "Loading transcription model...", "progress": 0})

    @patch("transcription_engine.AutoModelForSpeechSeq2Seq")
    @patch("transcription_engine.AutoProcessor")
    @patch("transcription_engine.torch")
    @patch("transcription_engine.librosa")
    def test_transcribe_file_creates_output(self, mock_librosa, mock_torch, mock_processor, mock_model, temp_dir):
    ...
    def test_transcribe_file_with_progress_callback(self, mock_torch, mock_processor, mock_model, temp_dir):
        ...
        with patch("transcription_engine.librosa") as mock_librosa, patch("transcription_engine.pipeline") as mock_pipeline:
