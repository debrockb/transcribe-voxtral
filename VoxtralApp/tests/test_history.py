...
class TestHistoryManagement:
    ...
    def test_upload_metadata(self, client, app, sample_audio_file):
        ...
        with open(sample_audio_file, "rb") as f:
            upload_data = {"file": (f, "metadata_test.mp3", "audio/mpeg"), "language": "en"}
...
class TestHistoryDownloads:
    ...
    def test_download_existing_upload(self, client, app, sample_audio_file):
        ...
        # Upload a file first
        with open(sample_audio_file, "rb") as f:
            upload_data = {"file": (f, "download_upload_test.mp3", "audio/mpeg"), "language": "en"}
...
class TestHistoryDeletion:
    ...
    def test_delete_single_upload(self, client, app, sample_audio_file):
        ...
        # Upload file first
        with open(sample_audio_file, "rb") as f:
            upload_data = {"file": (f, "delete_upload.mp3", "audio/mpeg"), "language": "en"}
            upload_response = client.post("/api/upload", data=upload_data, content_type="multipart/form-data")
    ...
    def test_delete_all_uploads(self, client, app, temp_dir):
        ...
        for i in range(3):
            ...
            with open(audio_file, "rb") as f:
                upload_data = {"file": (f, f"bulk_upload_{i}.mp3", "audio/mpeg"), "language": "en"}
                client.post("/api/upload", data=upload_data, content_type="multipart/form-data")
